...
function EmployWorkers() {
	...
	workerList[0].postMessage({ indexPerk: indexPerk, indexPerkRank: indexPerkRank, lock: getLockIndices(), legendaryWeapon: true });
	history.pushState(null, null, "/fr?" + encode([urlPerks, indexPerkRank, urlLocks]));
...
function generateDBurl(set, parentElem) {
	...
	if (perkList[set.cell[4].j][set.cell[4].k].type == weapon[set.id[4]][set.id[5]].cells[0])
		//if order is correct
		DBarray.push(perkList[set.cell[4].j][set.cell[4].k].DBmap, perkList[set.cell[5].j][set.cell[5].k].DBmap);
	else if (perkList[set.cell[4].j][set.cell[4].k].type == weapon[set.id[4]][set.id[5]].cells[1])
	...
	if (weapon[set.id[4]][set.id[5]].type == "Cestes aethériques") DBarray.push(3, 2, 0, 0, 0, 0);
	else if (weapon[set.id[4]][set.id[5]].type == "Hache") DBarray.push(5, 3, 0, 0, 0, 0);
	else if (weapon[set.id[4]][set.id[5]].type == "Épée") DBarray.push(7, 8, 0, 0, 0, 0);
	else if (weapon[set.id[4]][set.id[5]].type == "Chaînes-lames") DBarray.push(5, 3, 0, 0, 0, 0);
	else if (weapon[set.id[4]][set.id[5]].type == "Aetherolance") DBarray.push(5, 8, 0, 0, 0, 0);
	else if (weapon[set.id[4]][set.id[5]].type == "Marteau") DBarray.push(5, 1, 0, 0, 0, 0);
	else if (weapon[set.id[4]][set.id[5]].type == "Répéteurs") DBarray.push(0, 8, 9, 19, 0, 25);
...
function PerkClicked(event) {
	let perkRankString;
	if (event.target.classList.contains("plus6")) perkRankString = "6";
	else perkRankString = "3";
	...
	let perkCardTooltip = "";
	for (let i = 0; i < Object.keys(perkList[parseInt(event.target.id.split("_")[0])][parseInt(event.target.id.split("_")[1])].effects).length; i++)
		perkCardTooltip =
			perkCardTooltip +
			String(i + 1) +
			": " +
			perkList[parseInt(event.target.id.split("_")[0])][parseInt(event.target.id.split("_")[1])].effects[String(i + 1)].description +
			"\n";
	...
	divPerkCardTitle.appendChild(document.createTextNode("+" + perkRankString + " " + event.target.textContent));
	divPerkCardText.appendChild(divPerkCardTitle);
	//description
	divPerkCardContent = document.createElement("div");
	divPerkCardContent.appendChild(
		document.createTextNode(perkList[parseInt(event.target.id.split("_")[0])][parseInt(event.target.id.split("_")[1])].effects[perkRankString].description)
	);
...
function PlusButtonCLicked() {
	...
	event.target.parentElement.parentElement.children[1].children[1].children[1].textContent =
		...
		].effects[String(rankValue + 3)].description;
	EmployWorkers();
}
//-button
...
if (queryString[0] == "0") {
	...
	for (let i = 0; i < queryString[2].length; i++) document.getElementById(LockCheckboxIdList[queryString[2][i]]).click();
	EmployWorkers();
}
